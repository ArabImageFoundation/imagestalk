Build a set of highly [normalized](http://en.wikipedia.org/wiki/Database_normalization) tables. *Normalized* means, ideally, that no piece of data ever gets repeated, and every type of object has it's own set of columns that makes it independently searcheable. This schema is also flexible; if a new photo type needs to be added, it can be added by the user.

- The benefits:
    - (+) Semantic and categorized data (which opens the door to very granular searches)
    - (+) Customizable by the user
    - (+) Some queries *may* be more performant
    - (+) End-user has quite some flexibility in adding types
- The trade-offs:
    - (-) More difficult to build
    - (-) More difficult to debug
    - (-) Some queries *may* be slower (but can in most cases be optimized)
    - (-) If changes are required later, they may be very complex to implement (a lot of tables depend on each other)

---

the schema is available on [vertabelo](https://my.vertabelo.com/public-model-view/sGJx35gVGcUXDPHgPghsIiXvxr7wqxPZw6rnqLPno6SN3cLWnkHPL7BhFCbbfx3i?x=606&y=856&zoom=0.25)

image representation

[![complex design](./complex.png)](./complex.png)

zoomable representation

[[vertabelo("sGJx35gVGcUXDPHgPghsIiXvxr7wqxPZw6rnqLPno6SN3cLWnkHPL7BhFCbbfx3i?x=606&y=856&zoom=0.25")]]
